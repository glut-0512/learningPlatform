...
//获取PPT文件列表
function getSWFFileList(index,fileName){
...
function fillFileData(fileManager){
    ...
    var $li = $('<li></li>');
    var $divImg = $('<div class="media_img"><img src="img/icon_swf.png" /></div>');
    var $idvText = $('<div class="media_text"><h1>'+ fileManager.filename +'</h1><h2>面相对象：学生<span>资源简介：'+ fileManager.filename +'</span></h2><h3>关键词：</h3><h4>文件格式：swf</h4><h5>大小：未知</h5></div>');
    //下载按钮
